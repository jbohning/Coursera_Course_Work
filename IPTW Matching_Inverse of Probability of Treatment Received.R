...
#to get a weighted mean for a single covariate directly: (more brute force method)
mean(weight[treatment==1]*age[treatment==1])/(mean(weight[treatment==1]))

#get causal risk difference
glm.obj<-glm(died~treatment,weights=weight, data=mydata,
...
# fit propensity score model to get weights, but truncated (at the first 
#and 99th percentile, as seen in trunc=.01)
weightmodel<-ipwpoint(exposure= treatment, family = "binomial", link ="logit",
                      denominator= ~ age + female + meanbp1+ARF+CHF+Cirr+colcan+
